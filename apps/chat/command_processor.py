...
def get_closest_command(user_command: str) -> Optional[str]:
    ...
    matches = get_close_matches(user_command, SUPPORTED_COMMANDS, n=1, cutoff=0.6)
...
def analyze_freeform_message(user, message: str) -> str:
    ...
    if any(
        q in message_lower
        for q in ["what do you advice", "what should i do", "any suggestions"]
    ):
...
def analyze_productivity_trends(user) -> str:
    ...
    recent_tasks = Task.objects.filter(user=user, created_at__gte=five_days_ago)
...
def process_show_command(user, command: str, filter_text: str) -> str:
    ...
    # Define filters for different commands
    filters = {
    ...
    task_list = "\n".join(
        f"- {task.title} ({task.status}, {task.priority})" for task in sample_tasks
    )
...
def create_task(user, task_title: str) -> str:
    """Create a new task with AI-powered categorization and prioritization."""
    ...
    try:
        ...
        priority_response = gemini_client.models.generate_content(
            model="gemini-2.0-flash", contents=priority_prompt
        )
    ...
    try:
        ...
        difficulty_response = gemini_client.models.generate_content(
            model="gemini-2.0-flash", contents=difficulty_prompt
        )
    ...
    try:
        ...
        time_response = gemini_client.models.generate_content(
            model="gemini-2.0-flash", contents=time_prompt
        )
    ...
    if due_date.hour < 6:  # Before 6AM
        ...
        due_date = due_date.replace(hour=21, minute=0) + timedelta(
            days=1
        )  # Move to 9PM next day
    ...
    try:
        ...
        response = requests.post(
            "http://localhost:8000/api/tasks/", json=api_payload, headers=headers
        )
...
# Initialize ML models for task categorization
task_tokenizer = AutoTokenizer.from_pretrained(
    "vagrawal787/todos_task_model", local_files_only=False
)
...
task_model = torch.quantization.quantize_dynamic(
    task_model, {torch.nn.Linear}, dtype=torch.qint8
)
print("Model labels:", task_model.config.id2label)


def categorize_task(task_text: str) -> str:
    """Categorize task using ML model."""
    try:
        inputs = task_tokenizer(
            task_text, return_tensors="pt", truncation=True, padding=True
        )
